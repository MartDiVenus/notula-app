...
export function memoToMarkdown(memo: MemoItem): string {
  const recLabel = memo.repeatType === 'none' ? 'Puntuale' : `Ricorrente (${REPEAT_LABELS_IT[memo.repeatType] || memo.repeatType})`;
...
export function generateMemoPDF(memo: MemoItem): jsPDF {
  ...
  doc.setTextColor(100, 116, 139);
  doc.setFont('helvetica', 'bold');
  doc.text('CREATO IL:', margin + 6, y + 28);
  doc.setFont('helvetica', 'normal');
  doc.setTextColor(15, 23, 42);
  ...
  if (memo.groupId) {
    doc.setTextColor(100, 116, 139);
    doc.setFont('helvetica', 'bold');
    doc.text('GRUPPO ID:', margin + 92, y + 28);
  ...
  doc.text('DESCRIZIONE & NOTE:', margin, y);
  y += 5;
  ...
  const content = memo.description || '(Nessuna nota descrittiva inserita)';
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(9.5);
  ...
  // Riga 1 Footer
  doc.text(
    'Documento generato da Notula™ Memo Engine • Ideazione & Sviluppo Ing. Mario Fantini',
    margin,
    footerY
  );
...
export function parseXMLMemos(xmlText: string): MemoItem[] {
  ...
  for (let i = 0; i < memoNodes.length; i++) {
    ...
    const item: MemoItem = {
      ...
      title: getVal("title") || "Senza titolo",
...
export async function parseImportFile(file: File, masterPassword?: string | null): Promise<MemoItem[]> {
  ...
  if (isEncryptedString(content)) {
    if (!masterPassword) {
      const pwd = prompt("Inserisci la Master Password per decifrare il file importato:");
      if (!pwd) throw new Error("Master Password richiesta per file cifrati.");
