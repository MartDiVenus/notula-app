...
def replace_in_file(filepath, replacements):
    ...
    for it, en in replacements.items():
        # Match >Text<
        content = re.sub(f'>\s*{re.escape(it)}\s*<', f'>{{settings.language === "en" ? "{en}" : "{it}"}}<', content)
        # Match 'Text' or "Text" for standalone strings, but we must be careful
        # Instead, we will only replace >Text< to avoid breaking code.
...
for root, _, files in os.walk('src'):
    for file in files:
        if file.endswith('.tsx') or file.endswith('.ts'):
            ...
            # Before running blindly, we just do components
            if 'components' in filepath or 'App.tsx' in filepath:
